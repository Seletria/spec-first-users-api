...
### Data Storage
- PostgreSQL 16 running via Docker Compose (`docker-compose.yml`), with a named volume (`pgdata`) for persistence
- Data persists across API restarts and container restarts
- Schema is managed through migrations (`api/migrations/`, run with node-pg-migrate)
- Seed data is optional and dev-only: `npm run seed` (`api/scripts/seed.js`) inserts 3 sample users (Ayşe/Mehmet/Zeynep) via the same `createUser` repository function used by `POST /users` — not raw SQL, so validation/normalization/uniqueness rules apply identically
- The seed script is idempotent (`DuplicateEmailError` is caught and skipped on re-run) and is never a dependency for tests (hermetic test design — tests set up their own preconditions via the API)
- Seed data is excluded from CI

### ID Generation
- IDs are generated by the database via a `SERIAL` sequence (defined in the `create-users-table` migration)
- Sequence values are never reused, even after deletion
- No application-level ID logic
